select strftime('%m',"transaction".date)||'/1/'||strftime('%Y',"transaction".date),"transaction".desc,transaction_category.name,transaction_tag.name,"transaction".amount
from "transaction",transaction_rule,transaction_category,transaction_tag,transaction_ruletag
